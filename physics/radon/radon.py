...
class IRadon(nn.Module):
    ...
    def forward(self, x):
        ...
        reco = torch.zeros(x.shape[0], ch_size, it_size, it_size, device=x.device, dtype=self.dtype)
        reco_ = torch.zeros(x.shape[0],len(self.theta),it_size,it_size,device=x.device,dtype=self.dtype) #vvbp tensor

        for i_theta in range(len(self.theta)):
            reco += grid_sample(x, self.all_grids[i_theta].repeat(reco.shape[0], 1, 1, 1).to(x.device))
            reco_[:,i_theta,:,:] = grid_sample(x, self.all_grids[i_theta].repeat(reco.shape[0], 1, 1, 1).to(x.device))
        #pdb.set_trace()
        ...
        if not self.circle:
            ...
            reco = F.pad(reco, (-pad_width[0], -pad_width[1], -pad_width[0], -pad_width[1]))
            #pdb.set_trace()
            reco_ = F.pad(reco_, (-pad_width[0], -pad_width[1], -pad_width[0], -pad_width[1]))
        ...
        reco = reco * PI.item() / (2 * len(self.theta))
        reco_ = reco_ * PI.item() /(2 * len(self.theta))

        if self.out_size is not None:
            pad = (self.out_size - self.in_size) // 2
            reco = F.pad(reco, (pad, pad, pad, pad))
            reco_ = F.pad(reco_, (pad,pad,pad,pad))
    ...
    class IRadon(nn.Module):
        ...
        def forward(self, x):
            ...
            reco = torch.zeros(x.shape[0], ch_size, it_size, it_size, device=x.device, dtype=self.dtype)
            reco_ = torch.zeros(x.shape[0], len(self.theta), it_size, it_size, device=x.device,
                                dtype=self.dtype)  # vvbp tensor

            for i_theta in range(len(self.theta)):
                reco += grid_sample(x, self.all_grids[i_theta].repeat(reco.shape[0], 1, 1, 1).to(x.device))
                reco_[:, i_theta, :, :] = grid_sample(x, self.all_grids[i_theta].repeat(reco.shape[0], 1, 1, 1).to(
                    x.device))
            # pdb.set_trace()
